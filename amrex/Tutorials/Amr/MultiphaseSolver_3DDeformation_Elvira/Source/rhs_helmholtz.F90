...
contains
  ...
  integer, parameter :: nfine=20
  double precision :: G, x_here, y_here, z_here
        ...
        do i=lo(1),hi(1)
           x = dble(i)*dx(1) + prob_lo(1)
                do kk=1,nfine
                   do jj=1,nfine
                     do ii=1,nfine
                            x_here=x+real(ii,WP)/real(nfine+1,WP)*dx(1)
                            y_here=y+real(jj,WP)/real(nfine+1,WP)*dx(2)
                            z_here=z+real(kk,WP)/real(nfine+1,WP)*dx(3)
                            G  = 0.15_WP-sqrt((x_here-0.35_WP)**2+(y_here-0.35_WP)**2+(z_here-0.35_WP)**2)
                            if (G.gt.0.0_WP) then
                                VOF(i,j,k)=VOF(i,j,k)+1.0_WP/(real(nfine,WP)**3)
                            end if
                    end do
                end do
              end do
