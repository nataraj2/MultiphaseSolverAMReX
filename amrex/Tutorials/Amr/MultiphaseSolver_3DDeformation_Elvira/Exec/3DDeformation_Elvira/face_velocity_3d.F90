...
subroutine get_face_velocity(Grho,GrhoU,GrhoV,GrhoW,Lrho,LrhoU,LrhoV,LrhoW,VOF,clo,chi, &
     ...
     dx, prob_lo,time) bind(C, name="get_face_velocity")
  ...
  double precision, intent(in) :: dx(3), prob_lo(3), time
  ...
  do k = vx_l3, vx_h3
	 ...
	 x = (dble(i)+0.5d0) * dx(1) + prob_lo(1)
	   vx(i,j,k)=+2.0_WP*sin(M_PI*x)**2*sin(2.0d0*M_PI*y)*sin(2.0d0*M_PI*z)*cos(M_PI*time/3.0d0)
        end do
     end do
  end do
  ...
  do k = vy_l3, vy_h3
     ...
     do j = vy_l2, vy_h2
	   y = (dble(j)+0.5d0) * dx(2) + prob_lo(2)	
        do i = vy_l1, vy_h1
	 x = (dble(i)+0.5d0) * dx(1) + prob_lo(1)
        vy(i,j,k) = -sin(2.0d0*M_PI*x)*sin(M_PI*y)**2*sin(2.0d0*M_PI*z)*cos(M_PI*time/3.0d0)
        end do
     end do
  end do
 ...
 do k = vz_l3, vz_h3
    ...
    do j = vz_l2, vz_h2
       ...
       do i = vz_l1, vz_h1
	 x = (dble(i)+0.5d0) * dx(1) + prob_lo(1)
	  vz(i,j,k)= -sin(2.0d0*M_PI*x)*sin(2.0d0*M_PI*y)*sin(M_PI*z)**2*cos(M_PI*time/3.0d0)
       enddo
    enddo
 enddo	
