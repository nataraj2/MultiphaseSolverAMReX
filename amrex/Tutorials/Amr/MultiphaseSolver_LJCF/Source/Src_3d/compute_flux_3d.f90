  ...
  ! Allocate flux_hexahedron in IRL
   ...
   call new(f_moments)
    ...
    do       k = lo(3), hi(3)
	  ...
	  do ipt=1,8 
          projected_cell(:,ipt) =project(original_cell(:,ipt),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
	  enddo
	  ...
	  transported_cell(:,9)=0.25d0*[sum(transported_cell(1,1:4)),sum(transported_cell(2,1:4)),sum(transported_cell(3,1:4))]
          transported_cell(:,9)=project(transported_cell(:,9),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)  
   ...
   ! Last x face
    ...
    do  k = lo(3), hi(3)
	  ...
	  original_cell(:,7)=[x(i+1),y(j+1), z(k+1)];original_cell(:,8)=[x(i),y(j+1),  z(k+1)];
    	  ...
    	  ! Project all the cell nodal points just once
          projected_cell(:,1) =project(original_cell(:,1),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
          projected_cell(:,4) =project(original_cell(:,4),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
          projected_cell(:,8) =project(original_cell(:,8),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
          projected_cell(:,5) =project(original_cell(:,5),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
   ...
   do  k = lo(3), hi(3)
	  ...
	  original_cell(:,7)=[x(i+1),y(j+1), z(k+1)];original_cell(:,8)=[x(i),y(j+1),  z(k+1)];
    	  ...
    	  ! Project all the cell nodal points just once
          projected_cell(:,1) =project(original_cell(:,1),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
          projected_cell(:,2) =project(original_cell(:,2),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
          projected_cell(:,3) =project(original_cell(:,3),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
          projected_cell(:,4) =project(original_cell(:,4),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
   ...
   ! Last z face
    ...
    do  k = hi(3)+1, hi(3)+1
	  ...
	  original_cell(:,7)=[x(i+1),y(j+1), z(k+1)];original_cell(:,8)=[x(i),y(j+1),  z(k+1)];
          ...
          projected_cell(:,1) =project(original_cell(:,1),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
          projected_cell(:,5) =project(original_cell(:,5),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
          projected_cell(:,6) =project(original_cell(:,6),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
          projected_cell(:,2) =project(original_cell(:,2),-dt,i,j,k,umac,u_lo,u_hi,vmac,v_lo,v_hi,wmac,w_lo,w_hi,lo,hi,x,y,z,xm,ym,zm,dx)
...
subroutine compute_fluxes(i,j,k,transported_cell,pseparator,plocseplink,ploclink,lo,hi,x,y,z,xm,ym,zm,phi,gradxphi_mat,gradyphi_mat,gradzphi_mat,ph_lo,ph_hi,flux,volume)
 ...
 double precision :: centroid1(3),centroid2(3),Gbary(3),Lbary(3), plane(4)
...
! Get number of tags (elements) in the f_moments
  ...
  do n = 0,list_size-1
	...
	!if(isnan(getVolume(my_SepVM, 1)))cycle
	...
	if(isnan(getVolume(my_SepVM, 0)).or.isnan(getVolume(my_SepVM, 1)))then
                print*, getVolume(my_SepVM, 0), getVolume(my_SepVM, 1),phi_reconstruct(1)
                plane = getPlane(pseparator(ii,jj,kk),0)
                print*, plane, x(ii), y(jj), z(kk)
