...
subroutine compute_reconstruct(ilev,phiborder)
    implicit none
    integer :: ilev
    integer :: clo(4), chi(4), borlo(4), borhi(4), lo(4), hi(4)
    type(amrex_box) :: bx
    ...
    !do ilev = 0, amrex_get_finest_level()
	  ...
	  clo = lbound(gradxphi_ptr)
          ...
          call reconstruct(bx%lo,bx%hi,clo(1:3), chi(1:3), borlo, borhi, phiborder_ptr,  gradxphi_ptr, gradyphi_ptr, gradzphi_ptr, amrex_problo, amrex_geom(ilev)%dx)
       end do
  ...
  subroutine reconstruct(lo, hi, clo, chi, borlo, borhi, phiborder, gradxphi_mat, gradyphi_mat, gradzphi_mat, prob_lo, dx)
  implicit none

  integer, intent(in) :: lo(3), hi(3), clo(3), chi(3), borlo(3), borhi(3)
