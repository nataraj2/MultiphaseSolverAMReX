...
contains
  ...
  function project(p1,mydt,myi,myj,myk,umac,u_lo,u_hi,&
				       vmac,v_lo,v_hi,&
				       wmac,w_lo,w_hi,&
				       lo,hi,x,y,z,xm,ym,zm,dx) result(p2)
    implicit none
  ...
  function get_velocity(pos,i0,j0,k0,umac,u_lo,u_hi,&
				     vmac,v_lo,v_hi,&
				     wmac,w_lo,w_hi,&
                                     lo,hi,x,y,z,xm,ym,zm,dx) result(vel)
    implicit none
...
! Interpolate Uface velocity ------------------------------
    ...
    wx1=(pos(1)-x (i))/dx(1); wx2=1.0d0-wx1
    wy1=(pos(2)-ym(j))/dx(2); wy2=1.0d0-wy1
    wz1=(pos(3)-zm(k))/dx(3); wz2=1.0d0-wz1
...
! Interpolate Vface velocity ------------------------------
    ...
    do while (pos(3)-zm(k+1).ge.0.0d0.and.k+1.lt.kmaxo_); k=k+1; end do
    ! Prepare tri-linear interpolation coefficients
    wx1=(pos(1)-xm(i))/dx(1); wx2=1.0d0-wx1
    wy1=(pos(2)-y (j))/dx(2); wy2=1.0d0-wy1
    wz1=(pos(3)-zm(k))/dx(3); wz2=1.0d0-wz1
    ! Recast in array form
    ww(1,1,1)=wx2*wy2*wz2
    ww(2,1,1)=wx1*wy2*wz2
    ...
    wx1=(pos(1)-xm(i))/dx(1); wx2=1.0d0-wx1
    wy1=(pos(2)-ym(j))/dx(2); wy2=1.0d0-wy1
    wz1=(pos(3)-z (k))/dx(3); wz2=1.0d0-wz1
    ! Recast in array form
    ww(1,1,1)=wx2*wy2*wz2
    ww(2,1,1)=wx1*wy2*wz2
    ww(1,2,1)=wx2*wy1*wz2
