  ...
  subroutine tag_phi_error (level, time, lo, hi, phi, philo, phihi, vortmag, tag, taglo, taghi, phierr, &
    ...
    do       k = lo(3), hi(3)
       do    j = lo(2), hi(2)
		...
		if(phi(i,j,k,11).gt.VOFlo.and.phi(i,j,k,11).lt.VOFhi)then!.and.phi(i,j,k,11).le.VOFhi)then
		!if(phi(i,j,k,11).gt.0.01)then
		!if((z-1.0d0)*(z-2.0d0).le.0.0d0.and.abs(x).le.0.5d0)then
		!if(phi(i,j,k,11).gt.0.1d0)then
		tag(i-1:i+1,j-1:j+1,k-1:k+1) = settag
		endif
		
		!if(z.ge.0.0095d0.and.phi(i,j,k,11).gt.VOFlo.and.phi(i,j,k,11).le.VOFhi)then
		if(z.ge.0.0095d0)then!.and.phi(i,j,k,11).gt.VOFlo.and.phi(i,j,k,11).le.VOFhi)then
		!tag(i,j,k) = settag
		endif

             if (vortmag(i,j,k,1).ge.1e6)then
		!if((z-0.5d0)*(z-1.0d0).le.0.0d0.and.abs(x).le.0.5d0)then
		!if(phi(i,j,k,11).gt.0.0001.and.phi(i,j,k,11).lt.0.99)then!.and.phi(i,j,k,11).le.VOFhi)then
		!if(phi(i,j,k,11).gt.0.01)then
		!if((z-1.0d0)*(z-2.0d0).le.0.0d0.and.abs(x).le.0.5d0)then
		!if(phi(i,j,k,11).gt.0.1d0)then
		!tag(i,j,k) = settag
