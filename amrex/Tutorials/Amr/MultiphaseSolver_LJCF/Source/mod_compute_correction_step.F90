...
subroutine compute_correct_face_velocities
    ...
    integer :: ilev
    integer :: fxlo(4), fxhi(4), fylo(4), fyhi(4), fzlo(4), fzhi(4), prlo(4), prhi(4)
    type(amrex_box) :: bx
    type(amrex_mfiter) :: mfi
    real(amrex_real), contiguous, dimension(:,:,:,:), pointer :: Ufaceptr, dPdxptr, RHOxptr, Vfaceptr, dPdyptr, RHOyptr, Wfaceptr, dPdzptr, RHOzptr, Pptr
    integer :: nlevs
...
subroutine correct_face_velocities (level, lo, hi, Uface, dPdx, RHOx, fxlo, fxhi, &
  ...
  implicit none
  integer, intent(in) :: level, lo(3), hi(3), fxlo(3), fxhi(3), fylo(3), fyhi(3), fzlo(3), fzhi(3), prlo(3), prhi(3)
  ...
  do k=lo(3),hi(3)
     do j=lo(2),hi(2)
        do i=lo(1),hi(1)+1
                Uface(i,j,k) = Uface(i,j,k) - dt_step/dx(1)*(P(i,j,k)-P(i-1,j,k))/RHOx(i,j,k)
                !Uface(i,j,k) = Uface(i,j,k) - dt_step*dPdx(i,j,k)/RHOx(i,j,k)
        end do
     end do
  end do
  ...
  do k=lo(3),hi(3)
     do j=lo(2),hi(2)+1
        do i=lo(1),hi(1)
                Vface(i,j,k) = Vface(i,j,k) - dt_step/dx(2)*(P(i,j,k)-P(i,j-1,k))/RHOy(i,j,k)
                !Vface(i,j,k) = Vface(i,j,k) - dt_step*dPdy(i,j,k)/RHOy(i,j,k)
        end do
     end do
  end do
  ...
  do k=lo(3),hi(3)+1
     do j=lo(2),hi(2)
        do i=lo(1),hi(1)
                Wface(i,j,k) = Wface(i,j,k) - dt_step/dx(3)*(P(i,j,k)-P(i,j,k-1))/RHOz(i,j,k)
                !Wface(i,j,k) = Wface(i,j,k) - dt_step*dPdz(i,j,k)/RHOz(i,j,k)
        end do
     end do
  end do
...
subroutine compute_correct_energy
    ...
    integer :: ilev
    integer :: clo(4), chi(4), fxlo(4), fxhi(4), fylo(4), fyhi(4), fzlo(4), fzhi(4), prlo(4), prhi(4)
    type(amrex_box) :: bx
    type(amrex_mfiter) :: mfi
    real(amrex_real), contiguous, dimension(:,:,:,:), pointer :: phiptr, phi_mpptr, PXptr, Ufaceptr, PYptr, Vfaceptr, PZptr, Wfaceptr, Pptr, phi_nptr
    integer :: nlevs
    ...
    do ilev = 0, nlevs
	  ...
	  phi_mpptr => phi_mp(ilev)%dataptr(mfi)
          ...
          Wfaceptr  => facevelmfab(3,ilev)%dataptr(mfi)
	  Pptr 	    => Pmfab(ilev)%dataptr(mfi)
	  phi_nptr  => phi_n(ilev)%dataptr(mfi)
	  clo = lbound(phiptr)
	  chi = ubound(phiptr)
          fxlo = lbound(PXptr)
          fxhi = ubound(PXptr)
...
subroutine correct_energy (level, lo, hi, Grho, GrhoE, Lrho, LrhoE, RHO, clo, chi, PX, Uface, fxlo, fxhi,& 
  ...
  implicit none
  integer, intent(in) :: level, lo(3), hi(3), clo(3), chi(3), fxlo(3), fxhi(3), fylo(3), fyhi(3), fzlo(3), fzhi(3), prlo(3), prhi(3)
  double precision, dimension(clo(1):chi(1),clo(2):chi(2),clo(3):chi(3)) :: Grho, GrhoE, Lrho, LrhoE, RHO, VOF_n, VOF
  ...
  double precision, intent(in) :: dx(3), prob_lo(3)
  integer          :: i,j,k
  double precision :: x,y,z,pdeldotU

  do k=lo(3),hi(3)
     do j=lo(2),hi(2)
        do i=lo(1),hi(1)
		!pdeldotU = P(i,j,k)*( (Uface(i+1,j,k)-Uface(i,j,k))/dx(1)+&
                !                       (Vface(i,j+1,k)-Vface(i,j,k))/dx(2)+&
                !                       (Wface(i,j,k+1)-Wface(i,j,k))/dx(3) )
                !GrhoE(i,j,k) = GrhoE(i,j,k) - dt_step*pdeldotU - Grho(i,j,k)*dt_step/RHO(i,j,k)*( (PX(i+1,j,k)*Uface(i+1,j,k)-PX(i,j,k)*Uface(i,j,k))/dx(1)+&
		!				      (PY(i,j+1,k)*Vface(i,j+1,k)-PY(i,j,k)*Vface(i,j,k))/dx(2)+&
		!				      (PZ(i,j,k+1)*Wface(i,j,k+1)-PZ(i,j,k)*Wface(i,j,k))/dx(3) - pdeldotU )
		
	
		!LrhoE(i,j,k) = LrhoE(i,j,k) - dt_step*pdeldotU - Lrho(i,j,k)*dt_step/RHO(i,j,k)*( (PX(i+1,j,k)*Uface(i+1,j,k)-PX(i,j,k)*Uface(i,j,k))/dx(1)+&
		!				      (PY(i,j+1,k)*Vface(i,j+1,k)-PY(i,j,k)*Vface(i,j,k))/dx(2)+&
		!				      (PZ(i,j,k+1)*Wface(i,j,k+1)-PZ(i,j,k)*Wface(i,j,k))/dx(3) - pdeldotU )	
		GrhoE(i,j,k) = GrhoE(i,j,k) - dt_step*((1.0d0-VOF(i,j,k))*( (PX(i+1,j,k)*Uface(i+1,j,k)-PX(i,j,k)*Uface(i,j,k))/dx(1)+&
                                                     (PY(i,j+1,k)*Vface(i,j+1,k)-PY(i,j,k)*Vface(i,j,k))/dx(2)+&
                                                     (PZ(i,j,k+1)*Wface(i,j,k+1)-PZ(i,j,k)*Wface(i,j,k))/dx(3) ) )+ P(i,j,k)*(VOF_n(i,j,k)-VOF(i,j,k))

		LrhoE(i,j,k) = LrhoE(i,j,k) - dt_step*(VOF(i,j,k)*( (PX(i+1,j,k)*Uface(i+1,j,k)-PX(i,j,k)*Uface(i,j,k))/dx(1)+&
                                                     (PY(i,j+1,k)*Vface(i,j+1,k)-PY(i,j,k)*Vface(i,j,k))/dx(2)+&
                                                     (PZ(i,j,k+1)*Wface(i,j,k+1)-PZ(i,j,k)*Wface(i,j,k))/dx(3) ) ) + P(i,j,k)*(VOF(i,j,k)-VOF_n(i,j,k))
