...
contains
  ...
  subroutine my_filccn(compno,lo, hi, q, q_lo, q_hi, ncomp, domlo, domhi, dx, xlo, bc)
    ...
    real(amrex_real) :: Tinfty, Tj, Vj, Mj, rhoj, Vz, afac, rad, jetrad, Grho, GrhoU, GrhoV, GrhoW, GrhoE, Lrho, LrhoU, LrhoV, LrhoW, LrhoE
	...
	jetrad=0.00051d0
             ...
             ! Do nothing.
	    do k = lo(3), hi(3)
	       z=-0.0025d0+(dble(k)+0.5d0) * dx(3)
                do j = lo(2), hi(2)
		   y=-0.0035d0+(dble(j)+0.5d0) * dx(2)

		   rad = dsqrt(y**2+(z-0.002d0)**2)
		...
		if(rad.le.jetrad)then
                LrhoU=Lrho*20.0d0!*(1.0d0-rad/jetrad)**(1.0d0/7.0d0)
		else
		LrhoU=0.0d0
		endif
		LrhoV=0.0d0
		LrhoW=0.0d0
                LrhoE=(101325.0d0+4.4d0*6.0d8)/(3.4d0)+0.5d0*LrhoU**2/Lrho

                Grho=1.226d0
                GrhoU=0.0d0
		GrhoV=0.0d0
		GrhoW=Grho*300.0d0
                GrhoE=(101325.0d0)/(0.4d0)+0.5d0*(GrhoU**2+GrhoW**2)/Grho	
			 
  		        !if(compno.eq.1)then
			!q(i,j,k,n)=2.0d0*Grho - q(ilo,j,k,n)
			!elseif(compno.eq.2)then
			!q(i,j,k,n)=2.0d0*GrhoU - q(ilo,j,k,n)
			!elseif(compno.eq.3)then
			!q(i,j,k,n)=2.0d0*GrhoV - q(ilo,j,k,n)
			!elseif(compno.eq.4)then
			!q(i,j,k,n)=2.0d0*GrhoW-q(ilo,j,k,n)
			!elseif(compno.eq.5)then
			!q(i,j,k,n)=2.0d0*GrhoE-q(ilo,j,k,n)
			if(compno.eq.6)then
			if(rad.le.jetrad)then
                        q(i,j,k,n) = 2.0d0*Lrho - q(ilo,j,k,n)
			else
			q(i,j,k,n)=q(ilo,j,k,n)
			endif
			elseif(compno.eq.7)then
			if(rad.le.jetrad)then
			q(i,j,k,n) = 2.0d0*LrhoU - q(ilo,j,k,n)
			else
			q(i,j,k,n)=q(ilo,j,k,n)
			endif
			elseif(compno.eq.8)then
			if(rad.le.jetrad)then
			q(i,j,k,n)=2.0d0*LrhoV - q(ilo,j,k,n)
			else
			q(i,j,k,n)=q(ilo,j,k,n)
			endif
			elseif(compno.eq.9)then
			if(rad.le.jetrad)then
			q(i,j,k,n)=2.0d0*LrhoW - q(ilo,j,k,n)
			else
			q(i,j,k,n)=q(ilo,j,k,n)
			endif
			!elseif(compno.eq.10)then
		        !q(i,j,k,n) = 2.0d0*LrhoE - q(ilo,j,k,n)
			elseif(compno.eq.11)then
			if(rad.le.jetrad)then
			q(i,j,k,n)=2.0d0-q(ilo,j,k,n)
			elseif(rad.ge.jetrad.and.rad.le.1.2d0*jetrad)then
			q(i,j,k,n)=0.0d0
			else
			q(i,j,k,n)=q(ilo,j,k,n)
			endif
        ...
        do k = kmin, kmax
           do j = lo(2), hi(2)
              do i = lo(1), hi(1)
                      ...
                      Grho = 0.3d0!rhoj*(0.5d0*0.4d0*Vz*(1.0d0-Vz)*Mj**2+Vz+Tinfty/Tj*(1.0d0-Vz))**(-1)
                      GrhoW = Grho*700.0d0
                      GrhoE = 2.8d4/0.4d0 + 0.5d0*GrhoW**2/Grho

                      if(compno.eq.1)then
                      q(i,j,k,n) = 2.0d0*Grho - q(i,j,klo,n)
                        elseif(compno.eq.4)then
                        q(i,j,k,n) = 2.0d0*GrhoW - q(i,j,klo,n)
                        elseif(compno.eq.5)then
                      q(i,j,k,n) = 2.0d0*GrhoE - q(i,j,klo,n)
                        else
