...
contains
  ...
  subroutine my_filccn(compno,lo, hi, q, q_lo, q_hi, ncomp, domlo, domhi, dx, xlo, bc)
	...
	jetrad=0.0004d0
		...
		Lrho=1000.0d0
                LrhoU=Lrho*10.0d0!*(1.0d0-rad**2/jetrad**2)
		LrhoV=0.0d0
		LrhoW=0.0d0
        ...
        do k = kmin, kmax
           do j = lo(2), hi(2)
              do i = lo(1), hi(1)
                      ...
                      Grho = 1.226d0!rhoj*(0.5d0*0.4d0*Vz*(1.0d0-Vz)*Mj**2+Vz+Tinfty/Tj*(1.0d0-Vz))**(-1)
                      GrhoW = Grho*120.0d0
                      GrhoE = 101325.0d0/0.4d0 + 0.5d0*GrhoW**2/Grho
