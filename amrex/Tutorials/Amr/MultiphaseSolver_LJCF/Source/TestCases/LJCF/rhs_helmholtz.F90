...
do zcennum=3,3
  do xcennum=3,3
    do ycennum=3,3
      do k=lo(3),hi(3)
		...
		Lrho(i,j,k)=1000.0d0
		LrhoU(i,j,k)=Lrho(i,j,k)*10.0d0
		LrhoW(i,j,k)=Lrho(i,j,k)*0.0d0
		LrhoE(i,j,k)=(101325.0d0+4.4d0*6.0d8)/(3.4d0)+0.5d0*(LrhoU(i,j,k)**2+LrhoV(i,j,k)**2+LrhoW(i,j,k)**2)/Lrho(i,j,k)

                Grho(i,j,k)=1.226d0
                GrhoU(i,j,k)=Grho(i,j,k)*0.0d0
		!if(z.le.-0.001d0)then
                GrhoW(i,j,k)=Grho(i,j,k)*120.0d0
		!endif
		GrhoE(i,j,k)=101325.0d0/(0.4d0)+0.5d0*(GrhoU(i,j,k)**2+GrhoV(i,j,k)**2+GrhoW(i,j,k)**2)/Grho(i,j,k)
  ...
  do k = lo(3), hi(3)
       ...
       do j = lo(2), hi(2)
		...
		!	print*, rhoE_n(i,j,k), rhoU_n(i,j,k), rhoV_n(i,j,k), rhoW_n(i,j,k), gamma_mix_n, P_n, Pref_n, i, j, k
		!	stop
		!endif
