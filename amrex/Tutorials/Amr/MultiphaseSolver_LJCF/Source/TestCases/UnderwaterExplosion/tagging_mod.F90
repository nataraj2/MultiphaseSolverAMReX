...
contains
  ...
  subroutine tag_phi_error (level, time, lo, hi, phi, philo, phihi, vortmag, tag, taglo, taghi, phierr, &
    ...
    double precision :: rho_ijk, rho_im1jk, rho_ip1jk, rho_ijkm1, rho_ijkp1, ax,az, wx, wz, mean,wtilde
    ...
    do       k = lo(3)+1, hi(3)-1
       do    j = lo(2)+1, hi(2)-1
		...
		wx=abs(rho_ip1jk-2.0d0*rho_ijk+rho_im1jk)
		wz=abs(rho_ijkp1-2.0d0*rho_ijk+rho_ijkm1)
		mean=dsqrt((rho_ip1jk+2.0d0*rho_ijk+rho_im1jk)**2+(rho_ijkp1+2.0d0*rho_ijk+rho_ijkm1)**2)	
		wtilde=dsqrt(wx**2+wz**2)/(mean+1e-10)
