            ...
            if(abs(droprad-rad)<=dsqrt(delx**2+delz**2))then
              ...
              do kcell=1,nfine
                   do icell=1,nfine
                    ...
                    enddo
              enddo
                VOF(i,j,k)=1.0d0-vofcell/(delx*delz)
             endif
             ...
             if(droprad-rad>=dsqrt(delx**2+delz**2))then
                ...
                !VOF(i,j,k)=1.0d0-VOF(i,j,k)
                !VOF(i,j,k)=1.0d0
             !if(abs(x).le.1.0d0.and.abs(y).le.1.0d0.and.abs(z).le.1.0d0)VOF(i,j,k)=1.0d0
                ...
                if(z.le.-0.05d0)then
                Lrho(i,j,k)=1325.0d0
                LrhoW(i,j,k)=Lrho(i,j,k)*685.2d0
                LrhoE(i,j,k)=(1.915d9+4.4d0*6.0d8)/(3.4d0)+0.5d0*(LrhoW(i,j,k)**2)/Lrho(i,j,k)
                else
		Lrho(i,j,k)=1000.0d0
                LrhoW(i,j,k)=0.0d0
                LrhoE(i,j,k)=(101325.0d0+4.4d0*6.0d8)/(3.4d0)+0.5d0*(LrhoW(i,j,k)**2)/Lrho(i,j,k)
                endif
                !if((z-2.0d0)*(z-3.0d0).lt.1.0d0.and.abs(x).lt.1.0d0)then
                !       VOF(i,j,k)=0.0d0
                !endif
