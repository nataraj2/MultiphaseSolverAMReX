...
contains
  ...
  subroutine tag_phi_error (level, time, lo, hi, phi, philo, phihi, vortmag, tag, taglo, taghi, phierr, &
    ...
    real(amrex_real) :: dx(3), prob_lo(3), x, y, z, xcen, ycen, dircoord
       ...
       do    j = lo(2), hi(2)
          do i = lo(1), hi(1)
	        x = prob_lo(1) + dx(1) * (dble(i)+0.5d0)
                ...
                !xcen=-0.25d0*sin(2.0d0*3.1415d0*1.0d0*t_new(0))
                !ycen=0.25*cos(2.0d0*3.1415d0*1.0d0*t_new(0))
                !if(x.ge.xcen-0.16d0.and.x.le.xcen+0.16d0.and.y.ge.ycen-0.16d0.and.y.le.ycen+0.16d0)then
                if(phi(i,j,k,11).ge.0.0001.and.phi(i,j,k,11).le.0.9999)then
                tag(i,j,k) = settag
                endif
          enddo
       enddo
