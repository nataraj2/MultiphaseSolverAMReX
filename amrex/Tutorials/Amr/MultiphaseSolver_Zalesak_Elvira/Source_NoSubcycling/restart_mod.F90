...
implicit none
 ...
 interface
     ...
     subroutine countcellsatlevel(pba,cellcount) bind(c)
       import
       implicit none
       integer(c_int), intent(out) :: cellcount(*)
       type(c_ptr), value :: pba
     end subroutine countcellsatlevel 
 ...
 contains
  ...
  subroutine readcheckpointfile ()
     ...
     use amr_data_module, only : t_new, phi_new, phi_old, phi_n, gradxphi, gradyphi, gradzphi, flux_reg, grid_new, norm, ncomp, stepno_vec, do_reflux, dt_vec

     implicit none
     integer :: lev, finest_level(1), icomp
     ...
     do lev=0, finest_level(1)
        ...
        call amrex_multifab_build(phi_n(lev), ba(lev), dm(lev), ncomp, 0)

        if (lev > 0 .and. do_reflux) then
           call amrex_fluxregister_build(flux_reg(lev), ba(lev), dm(lev), amrex_ref_ratio(lev-1), lev, ncomp)
        end if
        call amrex_multifab_build(gradxphi(lev), ba(lev), dm(lev), ncomp, 0)
        call amrex_multifab_build(gradyphi(lev), ba(lev), dm(lev), ncomp, 0)
        call amrex_multifab_build(gradzphi(lev), ba(lev), dm(lev), ncomp, 0)
	call amrex_multifab_build(grid_new(lev), ba(lev), dm(lev), 0, 3)
        call amrex_multifab_build(norm(lev), ba(lev), dm(lev), 3, 3)
      enddo
