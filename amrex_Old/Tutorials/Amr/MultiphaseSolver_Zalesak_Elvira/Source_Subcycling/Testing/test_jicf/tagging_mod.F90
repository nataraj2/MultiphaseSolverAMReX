...
contains
  ...
  subroutine tag_phi_error (level, time, lo, hi, phi, philo, phihi, vortmag, tag, taglo, taghi, phierr, &
    ...
    real(amrex_real) , intent(in   ) :: vortmag(philo(1):phihi(1),philo(2):phihi(2),philo(3):phihi(3),ncomp)
    ...
    do       k = lo(3), hi(3)
       do    j = lo(2), hi(2)
		...
		!if(dsqrt(y**2+(z-100.0d0*t_new(0))**2).le.0.3d0.and.x.le.300.0d0*t_new(0))then
		!if(dsqrt(y**2+(z-x*1.0d0*t_new(0))**2).le.0.5d0.and.x.le.300.0d0*t_new(0))then

                if ((dsqrt(y**2+z**2).le.1.0d0.or.dsqrt(y**2+(z-x*5.0d0*t_new(0))**2).le.1.0d0) .and. x .le. 150.0d0*t_new(0))then
                !if (t_new(0).gt.1e-4.and.vortmag(i,j,k,1).ge.1000.0d0)then
		tag(i,j,k) = settag
		endif
