...
contains
  ...
  subroutine init_prob_start
    ...
    do ilev = 0, nlevs
	  ...
	  call init_prob_data(ilev, t_new(ilev), bx%lo, bx%hi, phi(:,:,:,1),phi(:,:,:,2),phi(:,:,:,3),phi(:,:,:,4),phi(:,:,:,5),lbound(phi), ubound(phi), &
  ...
  subroutine init_prob_data(level, time, lo, hi, &
     Grho, GrhoU, GrhoV, GrhoW, GrhoE, var_lo, var_hi,&
     dx, prob_lo) bind(C, name="initdata")

  implicit none
  integer, intent(in) :: level, lo(3), hi(3), var_lo(3), var_hi(3)
  double precision, intent(in) :: time
  double precision, intent(inout), dimension(var_lo(1):var_hi(1),var_lo(2):var_hi(2),var_lo(3):var_hi(3)) :: Grho,GrhoU,GrhoV,GrhoW,GrhoE
     ...
     do j=lo(2),hi(2)
        ...
        do i=lo(1),hi(1)
	      ...
	      if(x.le.0.0)then
              ...
              !GrhoV(i,j,k) = 0.0d0
              !GrhoW(i,j,k) = 0.0d0
              !GrhoE(i,j,k) = 100000.0/0.4
	      else
	      !Grho(i,j,k) = 0.125d0
              !GrhoU(i,j,k) = 0.0d0
              !GrhoV(i,j,k) = 0.0d0
              !GrhoW(i,j,k) = 0.0d0
              !GrhoE(i,j,k) = 10000.0/0.4
	      endif
	 enddo
      enddo
  enddo	
  ...
  do k=lo(3),hi(3)
   ...
   enddo
	      ...
	      !if(x**2+y**2.le.4.0d0)then!.and.y.ge.0.0d0)then
              !Grho(i,j,k) = 1.0d0
              !GrhoU(i,j,k) = 0.0d0
              !GrhoV(i,j,k) = 0.0d0
              !GrhoW(i,j,k) = 0.0d0
  ...
  do k=lo(3),hi(3)
     do j=lo(2),hi(2)
        ...
        do i=lo(1),hi(1)
           x = prob_lo(1) + (dble(i)+0.5d0) * dx(1)
                ...
                !Grho(i,j,k) = 1.226D0+0.01*101325/340.15518**2*exp(-log(2.0)*(x**2+y**2))
                !GrhoU(i,j,k) = 0.0D0
                !GrhoV(i,j,k) = 0.0D0
                !GrhoW(i,j,k) = 0.0D0
                !GrhoE(i,j,k) = 101325.0D0*(1.0+0.01*exp(-log(2.0)*(x**2+y**2)))/0.4

                !Grho(i,j,k) = 1.226D0+0.01*101325/340.15518**2*exp(-log(2.0)*(y**2+z**2))
                !GrhoU(i,j,k) = 0.0D0
                !GrhoV(i,j,k) = 0.0D0
                !GrhoW(i,j,k) = 0.0D0
                !GrhoE(i,j,k) = 101325.0D0*(1.0+0.01*exp(-log(2.0)*(y**2+z**2)))/0.4

	        !Grho(i,j,k) = 1.226D0+0.01*101325/340.15518**2*exp(-log(2.0)*(z**2+x**2))
                !GrhoU(i,j,k) = 0.0D0
                !GrhoV(i,j,k) = 0.0D0
                !GrhoW(i,j,k) = 0.0D0
                !GrhoE(i,j,k) = 101325.0D0*(1.0+0.01*exp(-log(2.0)*(z**2+x**2)))/0.4
	...
	!print*, "init...", prob_lo(2), prob_lo(3), dx(2), dx(3)
  ...
  do k=lo(3),hi(3)
	...
	Vz=0.5d0*(1.0d0+tanh(afac*(jetrad/rad-rad/jetrad)))
		...
		GrhoE(i,j,k) = 101325.0/0.4d0 + 0.5d0*(GrhoU(i,j,k)**2+GrhoV(i,j,k)**2+GrhoW(i,j,k)**2)/Grho(i,j,k)
                ...
                !Grho(i,j,k) = 1.226D0+0.01*101325/340.15518**2*exp(-log(2.0)*(x**2+y**2+z**2))
                !GrhoU(i,j,k) = 0.0D0
                !GrhoV(i,j,k) = 0.0D0
                !GrhoW(i,j,k) = 0.0D0
                !GrhoE(i,j,k) = 101325.0D0*(1.0+0.01*exp(-log(2.0)*(x**2+y**2+z**2)))/0.4
